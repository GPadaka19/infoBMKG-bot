# Data Peringatan/Warning Terbuka BMKG (CAP)

Data Peringatan Dini Cuaca (Nowcast) Terbuka BMKG telah tersedia di portal https://data.bmkg.go.id/peringatan-dini-cuaca dengan format XML Common Alerting Protocol (CAP). Berikut kode baris pemrograman PHP yang digunakan dalam mengolah data peringatan dini cuaca tersebut.

## Mengolah Data CAP Nowcast

#### Kode Baris PHP untuk Mengolah Data RSS Feed `nowcast-feed.php`
1. Berisi daftar peringatan dini cuaca aktif di tiap provinsi.
2. Terdapat tautan menuju detail CAP. Gunakan kode baris `nowcast-cap.php` untuk mengolah data detail CAP.
...
#### Kode Baris PHP untuk Mengolah Data Detail CAP `nowcast-cap.php`
1. Gunakan rujukan `urgency, severity, and certainty - (en-id).pdf` untuk referensi *alert code*.
2. Disertai peta Leaflet.js berbentuk polygon untuk wilayah terdampak peringatan dini cuaca.
3. Disertai infografik peringatan dini cuaca.
